...
class GraphNet(torch.nn.Module):
    ...
    def __init__(self, actions, num_feat, num_label, drop_out=0.6, multi_label=False, batch_normal=True, residual=True,
                 state_num=5, flag_se=False, short_cut=False):
        '''
        :param actions:
        :param multi_label:
        '''
        ...
        self.layer_nums = self.evaluate_actions(actions, state_num)
        self.n_out_all = 0

        # layer module
        self.build_model(actions, batch_normal, drop_out, num_feat, num_label, state_num, flag_se)

        #
        if self.short_cut:
            self.final_lin = nn.Linear(self.n_out_all, num_label, bias=False)
            gain = nn.init.calculate_gain('relu')
            nn.init.xavier_uniform_(self.final_lin.weight, gain=gain)

    def build_model(self, actions, batch_normal, drop_out, num_feat, num_label, state_num, flag_se):
        self.layers = torch.nn.ModuleList()
        self.gates = torch.nn.ModuleList()
        self.prediction = None
        self.build_hidden_layers(actions, batch_normal, drop_out, self.layer_nums, num_feat, num_label, state_num, flag_se)

    def evaluate_actions(self, actions, state_num):
    ...
    def build_hidden_layers(self, actions, batch_normal, drop_out, layer_nums, num_feat, num_label, state_num=6, flag_se=False):
        ...
        for i in range(layer_nums):
            ...
            combinator_type = actions[i * state_num + 2]
            act = actions[i * state_num + 3]
            head_num = actions[i * state_num + 4]
            out_channels = actions[i * state_num + 5]
            self.residual = False  # actions[i * state_num + 6]
            self.flag_se = actions[i * state_num + 6]
            ...
            self.layers.append(
                NASLayer(attention_type, aggregator_type, combinator_type, act, head_num, in_channels, out_channels, dropout=drop_out,
                         concat=concat, residual=residual, batch_normal=batch_normal, flag_se=self.flag_se))
            self.n_out_all += out_channels * head_num
    ...
    def forward(self, feat, g):
        ...
        if self.short_cut:
            for i, layer in enumerate(self.layers):
                h = layer(h, g).flatten(1)
                h_all = h if i == 0 else torch.cat((h_all, h), dim=1)
            output = self.final_lin(h_all)
        else:  #
            for i, layer in enumerate(self.layers):
...
class NASLayer(nn.Module):
    def __init__(self, attention_type, aggregator_type, combinator_type, act, head_num, in_channels, out_channels=8, concat=True,
                 dropout=0.6, pooling_dim=128, residual=False, batch_normal=True, flag_se=False):
        '''
        ...
        self.flag_se = flag_se and self.num_heads > 1

        if attention_type in ['cos', 'gen_linear']:
        ...
        for hid in range(self.num_heads):
            ...
            self.fnl.append(GATFinalize(hid, in_channels,
                                        out_channels, NASLayer.act_map(act),
                                        combinator_type, pooling_dim, residual))
        if self.flag_se:
            self.se = SELayer(self.num_heads)
    ...
    def aggregator_map(aggregator_type, in_dim, pooling_dim):
        ...
        elif aggregator_type == "pool_mean":
            return MeanPoolingAggregator(in_dim, pooling_dim)
        elif aggregator_type == "pool_max":
    ...
    def forward(self, features, g):
        ...
        # merge all the heads
        output = torch.stack(
            [g.ndata.pop('head%d' % hid) for hid in range(self.num_heads)], dim=1)
        if self.flag_se:
...
class MeanAggregator(nn.Module):
    ...
    def forward(self, neighbor):
        return torch.mean(neighbor, dim=1)
...
class GATFinalize(nn.Module):
    ...
    def __init__(self, headid, indim, hiddendim, activation, combinator_type, pooling_dim, residual):
        ...
        if self._comb_type == 'mlp':
            self.comb_fn = nn.Sequential(OrderedDict([
                ('enc', nn.Linear(indim, pooling_dim)),
                ('act1', nn.LeakyReLU()),
                ('dec', nn.Linear(pooling_dim, hiddendim)),
                ('act2', nn.LeakyReLU()),
            ]))
            nn.init.xavier_normal_(self.comb_fn.enc.weight, gain=1.414)
            nn.init.constant_(self.comb_fn.enc.bias, 0)
            nn.init.xavier_normal_(self.comb_fn.dec.weight, gain=1.414)
            nn.init.constant_(self.comb_fn.dec.bias, 0)
        elif self._comb_type == 'identity':
            if indim != hiddendim:
                self.comb_fn = nn.Linear(indim, hiddendim, bias=False)
                nn.init.xavier_normal_(self.comb_fn.weight, gain=1.414)
            else:
                self.comb_fn = Identity()
        elif self._comb_type == 'none':
            self.register_buffer('comb_fn', None)
        else:
    ...
    def forward(self, nodes):
        ...
        if self.comb_fn is not None:
            ret = self.comb_fn(nodes.data['h']) + ret
        '''
        if self.residual:
